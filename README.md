...
Cada resposta define quando **aquele card específico** pode reaparecer:

| Resposta | Efeito no Card | Delay Calculado |
|----------|---------------|-----------------|
| ✅ Fácil | `streak +2`, `erros -2` | `GLOBAL_CORRECT × 3` |
| 👍 Ok | `streak +2`, `erros -1` | `GLOBAL_CORRECT × 2` |
| 😢 Difícil | `streak +1` | `GLOBAL_CORRECT × 1` |
| 💀 Errei | `streak = 0`, `erros +1` | `GLOBAL_WRONG` |

> Exemplo: Se `GLOBAL_CORRECT = 1200s` (20min), um card marcado como "Fácil" só reaparece após ~60 minutos.

### 🔹 Ritmo Global Fixo
Enquanto cada card tem seu próprio agendamento, o Booster mantém um **metrônomo fixo** entre exibições:
- Após mostrar um card, o próximo só aparece após `GLOBAL_CORRECT` segundos
- Isso garante fluxo constante sem sobrecarregar o usuário
- Cards com `next_due` no futuro são automaticamente pulados

### 🔹 Ordenação Inteligente
Quando múltiplos cards estão disponíveis, o Booster escolhe o mais prioritário usando:

prioridade = (-erros_recentes, -streak, -bônus_favorito)

Ou seja: **erros recentes sobem**, **streak alto desce**, **favoritos ganham impulso**.

### 🔹 Buffer Rotativo Infinito
- `BUFFER_SIZE` cards ficam "ativos" na fila
- Ao responder, o card vai para o fim ou é substituído por um novo do pool
- Permite revisar centenas de cards/dia sem travar a memória ou a UI

### 🔄 Fluxo Visual

Anki DB → [Filtro: erros + favoritos] → Buffer Ativo → Exibição → Resposta
↓
Atualiza next_due + estado
↓
Rotação: fim da fila ou novo card

> 💡 **Resumo**: O Anki cuida do macro (revisão espaçada tradicional). O Booster cuida do micro (drill contínuo dos pontos fracos).
