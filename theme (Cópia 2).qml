...
Window {
    ...
    // Botão de Troca de Tema (Direita) - CICLA TODOS OS TEMAS
    MouseArea {
        z: 100
        width: 32; height: 32
        ...
        property var themesOrder: ["light", "medium", "dark"]
        property int currentIdx: 1 // Começa no "dark"

        onClicked: {
            currentIdx = (currentIdx + 1) % themesOrder.length
            bridge.setTheme(themesOrder[currentIdx])
        ...
        Text {
            ...
            anchors.centerIn: parent
            text: {
                if (t.bg === "#e5e7eb") return "🌞"
                if (t.bg === "#000000") return "🌒"
                return "🌗"
            }
            font.pixelSize: 16
            color: t.text
